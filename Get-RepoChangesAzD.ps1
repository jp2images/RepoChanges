...
# Read the PAT from a file that isn't in the Git Repo. Putting secrets in a 
# would create a potential security issue if the repo were made public 
# accidentally. PATs should also be per person and never shared to create a
# trainl for traceability if there were a security event.
$patFilePath = "$env:USERPROFILE\.ssh\Beckhoff-AzD-pat.txt"
$personalAccessToken = Get-Content -Path $patFilePath -Raw

# Define the file path to save the repo count
$repoCountFilePath = "$env:USERPROFILE\.ssh\repoCount.txt"
...
if ($verbose) {
    Write-Host ""
    Write-Host "REPOSITORY Count: $currentRepoCount"
    Write-Host "================================================================================"
}
...
foreach ($repo in $responseRepos.value) {
    ...
    Write-Host "Repository: ${repoName} with $countBranches branches"
    #   Write-Host "********************************************************************************"
    ...
    foreach ($branch in $responseBranches.value) {
        ...
        $responseCommits = Invoke-RestMethod -Uri $commitsUrl -Method Get -Headers @{Authorization = ("Basic {0}" -f $base64AuthInfo) }
        $commitCount = $responseCommits.count

        if ($commitCount -ne 0) {
            Write-Host "    COMMITS from $branchName"
            Write-Host "    -----"

            foreach ($commit in $responseCommits.value) {
                # Output the commits
                $responseCommits.value | ForEach-Object {
                    $commitId = $_.commitId
...
if ($currentRepoCount -eq $previclsousRepoCount) {
    Write-Host "Repository count has not changed"
}
else {
    Write-Host ""
    Write-Host "Repository count has changed from $previousRepoCount to $currentRepoCount"
}
